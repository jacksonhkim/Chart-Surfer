class Game {
    ...
    enterGame() {
        ...
        const startTarget = Math.floor(startBalance * 1.5);
    ...
    closePosition(price) {
        ...
        if (isProfit) {
            ...
            if (finalProfit >= 100000) {
                this.triggerFloatingText(`+ $${Math.floor(finalProfit).toLocaleString()}`, '#2ecc71');
            }
        } else {
            this.combo = 0;
            this.comboTimer = 0;
    ...
    addExp(amount) {
        ...
        if (leveledUp) {
            ...
            this.triggerLevelUp(newCap > prevCap); // 한도 증가 여부 전달
        }
        localStorage.setItem('cs_level', this.level);
        localStorage.setItem('cs_exp', this.exp);
    }

    triggerLevelUp(capIncreased) {
