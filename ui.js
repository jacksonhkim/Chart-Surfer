class UIManager {
    ...
    drawRealEstate(manager, player) {
        ...
        // 2행(4개)만 노출되도록 높이 고정 (230px)
        const listViewHeight = (cardH * 2) + gap;
        const listEndY = startY + listViewHeight;

        // 리스트 박스 배경
        this.ctx.fillStyle = 'rgba(0, 0, 0, 0.2)';
        this.ctx.fillRect(0, startY, w, listViewHeight);
        this.ctx.strokeStyle = 'rgba(255, 255, 255, 0.1)';
        this.ctx.strokeRect(0, startY, w, listViewHeight);
    ...
    checkRealEstateInput(x, y, manager) {
        ...
        const gap = 10;
        const listViewHeight = (cardH * 2) + gap;
        const listEndY = listStartY + listViewHeight;
